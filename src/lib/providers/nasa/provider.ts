...
async function requestNasa(searchText: string): Promise<readonly AssetSearchResult[]> {
  const response = await fetch(`/api/providers/nasa?q=${encodeURIComponent(searchText)}`);
  if (!response.ok) {
    const body = (await response.json().catch(() => null)) as { error?: string } | null;
    throw new Error(body?.error ?? "NASA Image Library is unavailable right now.");
  }
...
async function loadForText(searchText: string): Promise<readonly AssetSearchResult[]> {
  ...
  const promise = requestNasa(searchText)
    .then((assets) => {
...
/**
 * Live AssetSearchProvider backed by NASA's public Image and Video Library
 * search API. Architecturally identical to the other live-search providers:
 * real per-query text search, no results for an empty query.
 */
export const nasaSearchProvider: AssetSearchProvider = {
  id: "nasa-live",
  label: "NASA Image Library (live)",
